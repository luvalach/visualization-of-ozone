package cz.muni.fi.sdipr.visualizationofozone.rest.exception.handler;
...
public class RestExceptionHandler implements ExceptionMapper<Throwable> {
	...
	/**
	 * Intercepts exceptions and looking for ConstraintViolationException.
	 * ConstraintViolationException will be returned to client as json.
	 * 
	 * @param exception
	...
	public Response toResponse(Throwable exception) {
		...
		if (violationException != null) {
			...
			return Response.status(Status.BAD_REQUEST).entity(validationErrorDto).type(MediaType.APPLICATION_JSON)
					.build();
		} else {
			validationErrorDto.addFieldError("InternalServerError", exception.getMessage());
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity(validationErrorDto)
					.type(MediaType.APPLICATION_JSON).build();
		}
	}

	/**
	 * Goes recursively through exception causes and return
	 * ConstraintViolationException of null.
	 * 
	 * @param exception
