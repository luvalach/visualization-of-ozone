...
public class StationEndpoint {
	...
	public Response create(StationDTO dto) {
		Station entity = dto.fromDTO(null, em);
		em.persist(entity);
		return Response.created(
				UriBuilder.fromResource(StationEndpoint.class)
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Station> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT s FROM Station s WHERE s.id = :entityId ORDER BY s.id",
						Station.class);
	...
	@GET
	@Produces("application/json")
	public List<StationDTO> listAll(@QueryParam("start") Integer startPosition,
		...
		final List<Station> searchResults = findAllQuery.getResultList();
		final List<StationDTO> results = new ArrayList<StationDTO>();
		for (Station searchResult : searchResults) {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") Long id, StationDTO dto) {
		if (dto == null) {
